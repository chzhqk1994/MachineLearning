...
xy = np.loadtxt('train.txt', unpack=True, dtype='float32') # train.txt 를 전치시켜서 가져온다. 각 행당 [x0] [x1] [2].. 이렇게 들어간다
x_data = np.transpose(xy[0:3])  # 한 행에 [x0, x1, x2] 가 들어감, 즉 train.txt 상태 그대로 한 행씩 읽어옴
y_data = np.transpose(xy[3:])   # 한 행에 [yA yB yC] 가 들어감, np에 의해 train.txt 가 전치되어 들어왔으므로 소프트맥스 연산을 위해 이렇게 가져온다

X = tf.placeholder("float", [None, 3])  # [None, 3]  >> 행의 크기는 모르지만 열의 크기가 3인 행렬,  [None, None] 해도 무관
Y = tf.placeholder("float", [None, 3])  # "float" 말고 tf.float32 라고 써도 됨
